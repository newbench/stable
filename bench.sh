...
#
# Newbench
# Description: New Benchmark Script
# URL: https://raw.githubusercontent.com/newbench/stable/refs/heads/main/bench.sh
...
print_intro() {
    ...
    next
    echo " Version            : $(_green v2026.07.07)"
    echo " Global Speedtest   : $(_red "wget -qO- https://raw.githubusercontent.com/newbench/stable/refs/heads/main/bench.sh | bash")"
    echo " Region Speedtest   : $(_red "wget -qO- https://raw.githubusercontent.com/newbench/stable/refs/heads/main/bench.sh | bash -s -- -r <region>")"
    echo " iperf3 test        : $(_red "wget -qO- https://raw.githubusercontent.com/newbench/stable/refs/heads/main/bench.sh | bash -s -- -iperf")"
    echo " Ping & Routing     : $(_red "wget -qO- https://raw.githubusercontent.com/newbench/stable/refs/heads/main/bench.sh | bash -s -- -rt <region>")"
}
...
run_speed_sh() {
    ! _exists "wget" && _red "newbench is unable to run.\nError: wget command not found.\n" && kill -INT $$ && exit 1
    ! _exists "free" && _red "newbench is unable to run.\nError: free command not found.\n" && kill -INT $$ && exit 1
    ...
    if [ -n "$IPERF_ONLY" ]; then
        ...
        ! _exists "mtr" && _red "newbench is unable to run.\nError: mtr command not found.\n" && kill -INT $$ && exit 1
        speed
    else
        install_speedtest  
        speed 
    ...
    if [ -n "$2" ] && [[ "$2" != -* ]]; then
        ...
        case "$2" in
            ...
            *)
                ...
                echo "Visit newbench for instructions."
                exit 1
                ;;
        esac
        shift 2
...
if command -v curl >/dev/null; then
  share_link=$(curl -s -X POST -F 'file=@network-speed.txt' -F "region=$REGION" https://result.newbench/upload)
  if [ $? -ne 0 ]; then
